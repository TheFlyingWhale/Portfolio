/* colorController.js
  This script handels the color-mode(theme) and have two modes, light and dark.
*/
...
const secoundaryMidColor = 'rgb(35,35,35)'; //MidBlack - Used on elements above TintBlack background

const circGreenGradient = 'linear-gradient(#19FD5F, #47FD7F)'; //Linear green gradient
const circOrangeGradient = 'linear-gradient(#F38350 , #F06424)'; //Linear orange gradient
...
/**
 * buttonToNeg(element)
 * Replaces the postive style with the negative style on given elements
 */
buttonToNeg = element => {
    $(element).removeClass('posButton');
    $(element).addClass('negButton');
}

/**
 * buttonToPos(element)
 * Replaces the negative style with the positive style on given elements
 */
buttonToPos = element => {
    $(element).removeClass('negButton');
    $(element).addClass('posButton');
}

//Controlls animation of cmtCirc when cmtContainer is hovered
$(".cmtContainer").hover(() => {
